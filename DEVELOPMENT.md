...
This document describes the high level workflow used when working on the WP to Hootsuite WordPress Plugin.

You're free to use your preferred IDE and Git client.

## Prerequisites

If you haven't yet set up your local development environment with the WP to Hootsuite Plugin repository installed, refer to the [Setup Guide](SETUP.md).
...
- Shared WP Zinc libraries.

At present this Plugin has no runtime Composer dependencies; shared code is vendored directly in the `lib/shared`
and `lib/social` directories, which are kept in sync across the WP Zinc social Plugins.
