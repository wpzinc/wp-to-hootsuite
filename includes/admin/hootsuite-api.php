...
class WP_To_Social_Pro_Hootsuite_API {
    ...
    public function update_access_token() {
        ...
        if ( strtotime( '+5 minutes' ) < $this->token_expires ) {
            $this->base->get_class( 'log' )->add_to_debug_log( 'API: update_access_token(): Token expiry of ' . date( 'Y-m-d H:i:s', $this->token_expires ) . ' is not in the next 5 minutes. No need to refresh now.' );
            return false;
        }
        ...
        $this->base->get_class( 'log' )->add_to_debug_log( 'API: update_access_token(): Refresh Token Response: ' . print_r( $body, true ) );
        ...
        if ( ! $body->success ) {
            return new WP_Error(
                ...
                sprintf(
                    ...
                    __( 'Error refreshing access token.  Hootsuite said: %s', $this->base->plugin->name ),
                    $body->data
                )
            );
        }
    ...
    public function media_upload( $id, $media_url ) {
        ...
        if ( ! $file ) {
            return new WP_Error( 
                ...
                sprintf( 
                    ...
                    __( 'Attachment ID %1$s (%2$s) could not be found in the Media Library.', $this->base->plugin->name ),
                    $id,
                    $media_url
                )
            );
        ...
        // Upload Media
        $upload_result = wp_remote_request( $result->uploadUrl, array(
            'headers'   => array(
        ...
        if ( $this->base->get_class( 'settings' )->get_option( 'proxy', false ) ) {
            $response = wp_remote_get( $this->proxy_endpoint, array(
                ...
                'body'      => array(
                    ...
                    'params' => ( ! empty( $params ) ? ( $method == 'get' ? http_build_query( $params ) : json_encode( $params ) ) : '' ),
                )
            ) );
        } else {
            // Send request
    ...
    private function request_curl( $url, $method, $params, $timeout = 10 ) {
        ...
        if ( ! function_exists( 'curl_init' ) ) {
            return new WP_Error( 
                ...
                sprintf( 
                    /* translators: Social Media Scheduler Name (Buffer, Hootsuite, SocialPilot) */
                    __( '%s requires the PHP cURL extension to be installed and enabled by your web host.', $this->base->plugin->name ),
     ...
     * @return  mixed               WP_Error | object
     */
    private function parse_response( $response, $http_code, $params, $url ) {
        ...
        if ( is_null( $body ) ) {
            return new WP_Error(
                ...
                sprintf(
                    ...
                    __( 'Hootsuite API Error: %s returned a %s %s error. Please try again.', $this->base->plugin->name ),
                    $url,
                    $http_code,
                    $response
                )
        ...
        return new WP_Error( 
            ...
            sprintf( 
                ...
                __( 'Hootsuite API Error: #%1$s: %2$s', $this->base->plugin->name ),
                $http_code,
                implode( "\n", $message )
            )
        );
