...
class Admin_UI {
	...
	public function register_import_export_menu( $parent_slug = '' ) {
		...
		add_submenu_page( $slug, __( 'Import & Export', 'wp-to-hootsuite' ), __( 'Import & Export', 'wp-to-hootsuite' ), $minimum_capability, $this->plugin->name . '-import-export', array( $this, 'import_export_screen' ) );
	...
	public function register_support_menu( $parent_slug = '' ) {
		...
		add_submenu_page( $slug, __( 'Support', 'wp-to-hootsuite' ), __( 'Support', 'wp-to-hootsuite' ), $minimum_capability, $this->plugin->name . '-support', array( $this, 'support_screen' ) );
	...
	public function register_upgrade_menu( $parent_slug = '' ) {
		...
		add_submenu_page( $slug, __( 'Upgrade', 'wp-to-hootsuite' ), __( 'Upgrade', 'wp-to-hootsuite' ), $minimum_capability, $this->plugin->name . '-upgrade', array( $this, 'upgrade_screen' ) );
	...
	public function add_action_link( $links ) {
		...
		if ( $this->get_upgrade_url( 'plugins' ) ) {
			$links[] = '<a href="' . esc_attr( $this->get_upgrade_url( 'plugins' ) ) . '" rel="noopener" target="_blank">' . __( 'Upgrade', 'wp-to-hootsuite' ) . '</a>';
		}
	...
	public function maybe_display_footer_review_request( $text ) {
		...
		return sprintf(
			...
			__( 'Please rate %1$s %2$s on %3$s to help us grow %4$s. Thanks!', 'wp-to-hootsuite' ),
	...
	private function import() {
		...
		if ( ! is_array( $_FILES ) ) {
			$this->error_message = __( 'No file was uploaded', 'wp-to-hootsuite' );
			return;
		}
		if ( ! isset( $_FILES['import']['type'] ) || ! isset( $_FILES['import']['tmp_name'] ) || ! isset( $_FILES['import']['size'] ) ) {
			$this->error_message = __( 'Could not determine file type', 'wp-to-hootsuite' );
			return;
		}

		if ( isset( $_FILES['import']['error'] ) && $_FILES['import']['error'] !== 0 ) {
			$this->error_message = __( 'Error when uploading file.', 'wp-to-hootsuite' );
			return;
		}
		...
		switch ( $_FILES['import']['type'] ) {
			...
			case 'application/zip':
				...
				if ( $zip->open( sanitize_text_field( wp_unslash( $_FILES['import']['tmp_name'] ) ) ) !== true ) {
					$this->error_message = __( 'Could not extract the supplied ZIP file.', 'wp-to-hootsuite' );
					return;
				}
		...
		if ( ! is_array( $import ) ) {
			$this->error_message = __( 'Supplied file is not a valid JSON settings file, or has become corrupt.', 'wp-to-hootsuite' );
			return;
		}
		...
		$this->message = __( 'Settings imported.', 'wp-to-hootsuite' );
